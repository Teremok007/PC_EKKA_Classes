...
{ $Id: kpMatch.pas,v 1.3 1999-01-08 12:10:26-05 kp Exp kp $ }
...
implementation
  ...
  begin
     ...
     while (pattern[p] <> MATCH_CHAR_RANGE_CLOSE) do
      begin
        if (pattern[p+1]=MATCH_CHAR_RANGE) then
         begin
		...
		begin
			...
			begin
				...
				Case (pattern[p]) of
					MATCH_CHAR_KLEENE_CLOSURE:
                 begin
                    if (pattern[p+1]=MATCH_CHAR_RANGE_OPEN) then
                       getSet( CharSet )
                    else
                     begin
                       CharSet := [];
