...
const
  ...
  URI_CGI_CHK = '/cgi/chk';
  URI_DESC = '/desc';
  URI_PRINTFMREPORT='/cgi/proc/printfmreport';
  ...
  PRM_PRINTFMREPORT_REPID='RepID';
  PRM_PRINTFMREPORT_BEGINDATE='BeginDate';
  PRM_PRINTFMREPORT_ENDDATE='EndDate';
  PRM_PRINTFMREPORT_BEGINZREP='BeginZRep';
  PRM_PRINTFMREPORT_ENDZREP='EndZRep';

  PRINTREPORT_Z_REPORT = 0;
  PRINTREPORT_X_REPORT = 10;

  PRINTFMREPORT_FULL_BY_DATE=1;//полный отчет по датам
  PRINTFMREPORT_FULL_BY_Z=2;//полный отчет по номерам Z-отчетов
  PRINTFMREPORT_SHORT_BY_DATE=3;//краткий отчет по датам
  PRINTFMREPORT_SHORT_BY_Z=4;//краткий отчет по номерам Z-отчетов
  ...
  DEV_WRONG_BEGIN_DATE_CODE='7';
  DEV_WRONG_BEGIN_DATE_DESCRIPTION='Не корректная начальная дата';
  DEV_WRONG_END_DATE_CODE='8';
  DEV_WRONG_END_DATE_DESCRIPTION='Не корректная конечная дата';
  DEV_WRONG_BEGIN_Z_CODE='9';
  DEV_WRONG_BEGIN_Z_DESCRIPTION='Не корректный начальный номер Z-отчета';
  DEV_WRONG_END_Z_CODE='10';
  DEV_WRONG_END_Z_DESCRIPTION='Не корректный конечный номер Z-отчета';
  DEV_UNKNOWN_PRINTFMREPORT_ERROR_CODE='11';
  DEV_UNKNOWN_PRINTFMREPORT_ERROR_DESCRIPTION='При печати фискального отчета произошла не известная ошибка';
...
function PrintCheck(const txt: string; out ErrorCode, ErrorDescription: string; out ResponsedReceipt: T_cgi_chk_object; pIPAddr: string; PayType: byte = 1; rrn_: integer = 0; card_: string = ''): boolean;         //печать чека
...
function PrintFMReport(pIPADDR:string;out ErrorCode,ErrorDescription:string;const RepID:Byte; BegDate:TDateTime=0; EndDate:TDateTime=0; BegZRep:integer=0; EndZRep:integer=0):boolean;//отчеты из фискальной памяти
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          cl:=sl.Strings[i];
          Delete(cl,1,length('content-length:')+1);
  ...
  cgi_dev_info.prot:=0;
{
  ...
  try
    ...
    try
      StrToInt64(cgi_dev_info.dev_fn);
    except
      cgi_dev_info.dev_fn:='0';
    end;
  finally
...
(*
function ParsePrintFMReport(const js:string):boolean;
var
  i:integer;
  ErrorString:string;
  IsArray:boolean;
begin
  ...
  begin
    try
      ...
      if IsArray then
      begin
        with TJSONArray.create(ErrorString) do
        begin
          try
            SetLength(_Error,length);
            for i:=0 to length-1 do
            begin
              with TJSONObject.create(getString(i)) do
              begin
                try
                  if has('e') then _Error[i].e:=getString('e');
                finally
                  destroy;
                end;
              end;
      ...
      begin
        if system.length(trim(ErrorString))>0 then
          if ErrorString[1]='{' then
          begin
            with TJSONObject.create(ErrorString) do
            begin
              try
                SetLength(_Error,1);
                if has('e') then _Error[0].e:=getString('e');
              finally
                destroy;
              end;
            end;
      ...
      if system.Length(_Error)>0 then Result:=true;
    finally
      destroy;
    end;
  end;
...
function ParseCheckListRequest(const js: string; const flFiscal: boolean = true): boolean;
  ...
  procedure setCgiChkObjectEmpty(const arrElemID: integer);
  begin
    with cgi_chk.cgi_chk_object[arrElemID] do
    begin
      L:=nil;
      Z1:=0;
      F:=nil;
    ...
    try
      ...
      begin
        ...
        begin
          jso:=TJSONObject.create(jsa.getString(i));
          try
            setCgiChkObjectEmpty(i);
            if jso.has('Z1') then
            begin
              for j:=0 to i do setCgiChkObjectEmpty(j);
              Continue;
            end;
            ...
            begin
              if jso.has('no') then cgi_chk.cgi_chk_object[i].no:=jso.getInt('no') else cgi_chk.cgi_chk_object[i].no:=0; //для объектов L, F, R, IO - номер чека
        ...
        for i:=0 to length(cgi_chk.cgi_chk_object)-1 do
          if flFiscal then
          begin
            if (cgi_chk.cgi_chk_object[i].no > cgi_chk.max_check_no)and(cgi_chk.cgi_chk_object[i].no < MAX_RECEIPT_NUMBER) then
              cgi_chk.max_check_no:= cgi_chk.cgi_chk_object[i].no
          end
          else
            if (cgi_chk.cgi_chk_object[i].no > cgi_chk.max_check_no) then
              cgi_chk.max_check_no:= cgi_chk.cgi_chk_object[i].no;
        cgi_chk.ErrorCode:='';
...
procedure parseErrorResponseString(const errJsonString: string; out errCode,errString: string);
begin
end;
...
begin
  ...
  begin
    try
//      if has('err') then parseErrorResponseString(getString('err'),errCode,errString);
...
function decompressDeflatedStream(const V: variant; out deflated: boolean): WideString;
var
  compressedStream: Variant;
  os: TOLEStream;
...
begin
  try
    compressedStream:=V;
    TVarData(compressedStream).vType:=varUnknown;
    os:=TOLEStream.Create(IStream(TVarData(compressedStream).VUnknown));
    ...
    200..299:
      begin
        ...
        case Req.Status of //get
          ...
          200..299:
            begin
              deflatedString:=decompressDeflatedStream(req.ResponseStream,IsDeflated);

              if IsDeflated then
                decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,4,length(deflatedString)-3)))
              else
                decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,0,length(deflatedString))));
              decodedString:=UTF8ToAnsi(decodedString);
              if ParseStateRequest(decodedString,ErrorCode,ErrorDescription) then
              begin
                ErrorCode:=cgi_state.ErrorCode;
          ...
          200..299:
            begin
              ...
              if IsDeflated then
                ...
                decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,0,length(deflatedString))));
              decodedString:=UTF8ToAnsi(decodedString);
              if ParseDeviceInfoRequest(decodedString) then
              begin
          ...
          200..299:
            begin
              ...
              if IsDeflated then
                ...
                decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,0,length(deflatedString))));
              decodedString:=UTF8ToAnsi(decodedString);
              if ParseStatusRequest(cgi_status,decodedString) then
              begin
                Result:=true;
                exit;
              end
          ...
          200..299:
            begin
              ...
              if IsDeflated then
                ...
                decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,0,length(deflatedString))));
              decodedString:=UTF8ToAnsi(decodedString);
              if ParseProcGetJrnRoom(decodedString,ErrorCode,ErrorDescription) then
              begin
                Result:=true;
            ...
            begin
{
    Типы оплат:
     ...
     4 - Наличные  |   1  | Оплата Наличными
              ...
              if PayType = 4 then
              begin
                if _no<>0 then js:=js+',"no":'+IntToStr(_no) else js:=js+',"no":'+IntToStr(PayType);
                if rrn_>0 then js:=js+',"rrn":'+IntToStr(rrn_);
                if trim(card_)<>'' then js:=js+',"card":"'+card_+'"';
              end
              else
              begin
                if _no<>0 then js:=js+',"no":'+IntToStr(_no) else js:=js+',"no":'+IntToStr(PayType);
          ...
          200..299:
            begin
              ...
              if IsDeflated then
                ...
                decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,0,length(deflatedString))));
              decodedString:=UTF8ToAnsi(decodedString);
              if ParsePrintZXReport(decodedString) then
              begin
                ErrorCode:=DEV_STATE_ERROR_CODE_CANNOT_PRINT_CHECK;
                ErrorDescription:=DEV_STATE_ERROR_DESCRIPTION_CANNOT_PRINT_CHECK;
                Result:=false;
          ...
          200..299:
            begin
              ...
              if IsDeflated then
                ...
                decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,0,length(deflatedString))));
              decodedString:=UTF8ToAnsi(decodedString);
              if ParseSetClock(decodedString) then
              begin
          ...
          200..299:
            begin
              ...
              if IsDeflated then
                ...
                decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,0,length(deflatedString))));
              decodedString:=UTF8ToAnsi(decodedString);
              if ParseSoundRequest(decodedString) then
              begin
          ...
          200..299:
            begin
              ...
              if IsDeflated then
                ...
                decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,0,length(deflatedString))));
              decodedString:=UTF8ToAnsi(decodedString);
              if ParseCheckListRequest(decodedString) then
              begin
                ...
                try
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        if cgi_chk.cgi_chk_object[i].F[j].P.no = 1 then
                          ReceivedFromCustomer:=ReceivedFromCustomer+cgi_chk.cgi_chk_object[i].F[j].P.sum;
                    ...
                    begin
                      ...
                      begin
                        if cgi_chk.cgi_chk_object[i].R[j].P.no = 1 then
                          IssuedToCustomer:=IssuedToCustomer+cgi_chk.cgi_chk_object[i].R[j].P.sum;
                    ...
                    begin
                      for j:=0 to length(cgi_chk.cgi_chk_object[i].F)-1 do
                      begin
                        if (cgi_chk.cgi_chk_object[i].F[j].P.no = 2)or(cgi_chk.cgi_chk_object[i].F[j].P.no = 3)or(cgi_chk.cgi_chk_object[i].F[j].P.no = 4) then
                          NonCashPayment:=NonCashPayment+cgi_chk.cgi_chk_object[i].F[j].P.sum;
                      end; //for j:=0 to length(cgi_chk.cgi_chk_object[i].F) do
                    end; //if cgi_chk.cgi_chk_object[i].F <> nil then
                    ...
                    begin
                      for j:=0 to length(cgi_chk.cgi_chk_object[i].R)-1 do
                      begin
                        if (cgi_chk.cgi_chk_object[i].R[j].P.no = 2)or(cgi_chk.cgi_chk_object[i].R[j].P.no = 3)or(cgi_chk.cgi_chk_object[i].R[j].P.no = 4) then
                          NonCashRefund:=NonCashRefund+cgi_chk.cgi_chk_object[i].R[j].P.sum;
                      end; //for j:=0 to length(cgi_chk.cgi_chk_object[i].R) do
                    end; //if cgi_chk.cgi_chk_object[i].R <> nil then
                  end;
                  ...
                  if StartingBalance <> 0 then ch[0]:=IntToStr(trunc(StartingBalance/0.01)) else ch[0]:='0'; //0 начальный остаток
                  if ServiceAppendix <> 0 then ch[1]:=IntToStr(trunc(ServiceAppendix/0.01)) else ch[1]:='0'; //1 служебное внесение
                  if ServiceSeizure <> 0 then ch[2]:=IntToStr(trunc(ServiceSeizure/0.01)) else ch[2]:='0'; //2 служебное изъятие
                  if ReceivedFromCustomer <> 0 then ch[3]:=IntToStr(trunc(ReceivedFromCustomer/0.01)) else ch[3]:='0'; //3 получено от клиента нал.
                  if IssuedToCustomer <> 0 then ch[4]:=IntToStr(trunc(IssuedToCustomer/0.01)) else ch[4]:='0'; //4 выдано клиенту нал.
                  if FinalBalance <> 0 then ch[5]:=IntToStr(trunc(FinalBalance/0.01)) else ch[5]:='0'; //5 конечный остаток
                  if NonCashPayment <> 0 then ch[6]:=IntToStr(trunc(NonCashPayment/0.01)) else ch[6]:='0'; //6 безналичная оплата
                  if NonCashRefund <> 0 then ch[7]:=IntToStr(trunc(NonCashRefund/0.01)) else ch[7]:='0'; //7 безналичный возврат
                  CshStt:=ch.Text;
                finally
                  ch.Free;
                end;
...
begin
  ...
  u:=PROTOCOL+pIPAddr+URI_PRINTFMREPORT;
  if (RepID<>PRINTFMREPORT_FULL_BY_DATE)and(RepID<>PRINTFMREPORT_FULL_BY_Z)and(RepID<>PRINTFMREPORT_SHORT_BY_DATE)and(RepID<>PRINTFMREPORT_SHORT_BY_Z) then
  begin
    ErrorCode:=DEV_UNKNOWN_PRINTFMREPORT_ERROR_CODE;
    ErrorDescription:=DEV_UNKNOWN_PRINTFMREPORT_ERROR_DESCRIPTION;
    Result:=false;
    exit;
  end;

  case RepID of
    PRINTFMREPORT_FULL_BY_DATE,PRINTFMREPORT_SHORT_BY_DATE://отчет по дате
      begin
        if (BegDate=0)or(EndDate=0) then
        begin
          if BegDate=0 then
          begin
            ErrorCode:=DEV_WRONG_BEGIN_DATE_CODE;
            ErrorDescription:=DEV_WRONG_BEGIN_DATE_DESCRIPTION;
          end;
          if EndDate=0 then
          begin
            ErrorCode:=DEV_WRONG_END_DATE_CODE;
            ErrorDescription:=DEV_WRONG_END_DATE_DESCRIPTION;
          end;
          Result:=false;
          exit;
        end;
      end;
    PRINTFMREPORT_FULL_BY_Z,PRINTFMREPORT_SHORT_BY_Z://отчет по номерам Z-отчетов
      begin
        if (BegZRep=0)or(EndZRep=0) then
        begin
          if BegZRep=0 then
          begin
            ErrorCode:=DEV_WRONG_BEGIN_Z_CODE;
            ErrorDescription:=DEV_WRONG_BEGIN_Z_DESCRIPTION;
          end;
          if EndZRep=0 then
          begin
            ErrorCode:=DEV_WRONG_END_Z_CODE;
            ErrorDescription:=DEV_WRONG_END_Z_DESCRIPTION;
          end;
          Result:=false;
          exit;
        end;
  ...
  if (BegDate=0)and(EndDate=0) then
  begin
    BegDate:=Date();
    EndDate:=Date();
  end;
  str_params:='?'+IntToStr(RepID)+
    ...
    200..299:
      begin
        ...
        if IsDeflated then
          ...
          decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,0,length(deflatedString))));
        decodedString:=UTF8ToAnsi(decodedString);
        if ParsePrintFMReport(decodedString,ErrorCode,ErrorDescription) then
        begin
          ErrorCode:=DEV_STATE_ERROR_CODE_CANNOT_PRINT_CHECK;
          ErrorDescription:=DEV_STATE_ERROR_DESCRIPTION_CANNOT_PRINT_CHECK;
          Result:=false;
