...
Type TArrStr=Array of OleVariant;
     ...
     public
       ...
       function  fpAddBack(
                           Name:String;                             // Регистрация позиции возврата
                           Kol:Integer;
                           Cena:Currency;
                           Divis:Byte;
                           Artic:Integer;
...
//                       ShowMessage(CurrToStrF((StrToInt(FRD_Item[3])-StrToInt(FRD_Item[4])+StrToInt(FRD_Item[6])-StrToInt(FRD_Item[7]))*0.01,ffFixed,2));
                       ...
                       if Not Result then Abort;
                       if sm='0' then ri.Add(IntToStr(StrToInt(RD_Item[4])+1)) // Номер последнего Z-отчета
...
//                   SaveRegisters;
                   ...
                   if Not CheckErrorExellio then
                    begin
                     if Pos(AnsiUpperCase('Выполнение команды недопустимо в текущем фискальном режиме работы'),AnsiUpperCase(LastErrorDescr))<>0 then ss:='0' else Abort;
                    end; 
  ...
  if EmulEKKA then
   try
    ...
    if FIsCopy then
     ...
     end else MrFont.AddStrC('чек №  '+IntToStr(Qr.FieldByName('NLastChek').AsInteger+1),1);
...

{
    ...
    if DiscSum<0 then
     MrFont.AddStr2J('Знижка -' ,' -'+CurrToStrF_(Abs(DiscSum))+'  ',0) else
    if DiscSum>0 then
                   ...
                   if FBNumb_chek=1 then
                    begin
                     if SumB1<FSumSale-FSumVoid then
                      ...
                      if IsDnepr then MrFont.AddStr2J('Безготiвкова.3',' '+CurrToStrF_(Abs(SumB1)),0)
                                 else MrFont.AddStr2J('Безготiвкова.1',' '+CurrToStrF_(Abs(SumB1)),0);
                      if SumS>0 then
                       begin
                        MrFont.AddStr2J('Готiвки',' '+CurrToStrF_(Abs(TakedSumm)),1);
                               ...
                               if SumB1<FSumVoid then
                                begin
                                 ...
                                 if IsDnepr then MrFont.AddStr2J('Безготiвкова.3',' '+CurrToStrF_(Abs(SumB1)),0)
                                          ...
                                          if IsDnepr then MrFont.AddStr2J('Безготiвкова.3',' '+CurrToStrF_(Abs(SumB1-FSumVoid)),0)
                                                     else MrFont.AddStr2J('Безготiвкова.1',' '+CurrToStrF_(Abs(SumB1-FSumVoid)),0);
                                         end
                              end
    ...
    MrFont.AddStrC('Бажаємо вам здоров''я',1);
     ...
     if FIsCopy=False then
      begin
       ...
       if IsFLP=False then
        MrFont.AddStr('#'+AddStr_(IntToStr(Qr.FieldByName('NLastChek').AsInteger),'_',10)+' '+DateTimeToStrSlash(FDtChek)+' ФН:'+FsFN,0);
      end else begin
                if IsFLP=False then
  ...
  Result:=False;
   {
    ...
    Типы оплат:
          Мария    | Датекс FP3530 | Exellio LP1000 | Exellio FP2000 | Описание
     1 - Безнал 3  |      N        |       2        |        4       | Оплата в кредит
     2 - Безнал 2  |      D        |       4        |        3       | Оплата с помощью платежной карты
     3 - Безнал 1  |      C        |       3        |        2       | Оплата чеком
     4 - Наличные  |      P        |       1        |        1       | Оплата Наличными
...
var S:String;
{
 ...
 begin
  if (Param in [1,2]) and ((AptekaID in [240]) or (IsFLP)) then S:='НЕ ФIСКАЛЬНИЙ ЧЕК' else
   Case Param of
    1:  S:='ФIСКАЛЬНИЙ ЧЕК';
    2,4:S:='!  НЕ  ФIСКАЛЬНИЙ  !';
    3:  S:='';
  ...
  try
   ...
   if (IsOpen=True) or (Param=1) then
    begin
     MrFont.AddStr(' останнiй фiскальний чек '+DateTimeToStrSlash(Qr.FieldByName('DtLastChek').AsDateTime),0);
     MrFont.AddStr(' змiна вiдкрита          '+DateTimeToStrSlash(Qr.FieldByName('DtOpenSmena').AsDateTime),0);
    end else MrFont.AddStrC('змiна не вiдкрита',1);
   ...
   if (IsOpen=False) and (Param=0) then
    begin
     ...
     MrFont.AddStrC(' - - - - - - - - - -',1);
              ...
              MrFont.AddStr('пдв                 *А 20.00%',0);
              if NumZ>0 then MrFont.AddStrC('діє з 31/10/14, з №Z 0001',0)
                        else MrFont.AddStrC('діє з 31/10/14, з №Z 0001',0);
              MrFont.AddStr2J('база оподаткування',CurrToStrF_(Qr.FieldByName('SumA').AsCurrency),0);
              MrFont.AddStr2J('сумма податку вiд бази',CurrToStrF_(Qr.FieldByName('Sum_nAb').AsCurrency),0);
              MrFont.AddStr2J('сумма податку по чекам',CurrToStrF_(Qr.FieldByName('Sum_nA').AsCurrency),0);
              ...
              MrFont.AddStr2J('безготівкова.2',CurrToStrF_(Qr.FieldByName('Sum7').AsCurrency-Qr.FieldByName('SumB1').AsCurrency),0);
              MrFont.AddStr2J('кількість чеків',Qr.FieldByName('CntCheks').AsString,0);
