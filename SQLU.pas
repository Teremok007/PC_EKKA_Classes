...
function GetNaklSps(Qr:TADOQuery; Apt_ID:String; D1,D2:TDateTime; Medoc:Byte=0):Boolean;
function GetNaklSpsNDS(Qr:TADOQuery; Apt_ID:String; D1,D2:TDateTime; Medoc:Byte=0):Boolean;
...
function GetNaklSps(Qr:TADOQuery; Apt_ID:String; D1,D2:TDateTime; Medoc:Byte=0):Boolean;
 begin
  try
   ...
   Qr.SQL.Add('       (case when b.sp3909 in (''    1F'',''     7'',''    16'',''    2F'') or s.dt is not null then 1 else 0 end) as NReady ');
   Qr.SQL.Add('from                                 ');

   Qr.SQL.Add('     '+Opt.Gamma+'.._1sjourn j (nolock)                                                                         ');
   Qr.SQL.Add('     inner join                                                                                         ');
   Qr.SQL.Add('     '+Opt.Gamma+'..dh3901 b (nolock) on b.iddoc = j.iddoc                                                      ');
   Qr.SQL.Add('	       left join (select skl,id_apteka,dt,medoc from workwith_gamma..scan_corr group by skl,id_apteka,dt,medoc) s  ');
   Qr.SQL.Add('		               on IsNull(s.medoc,0)>='+IntToStr(Medoc)+' and b.sp9272=s.skl and b.sp3905=s.id_apteka and s.dt=convert(datetime,convert(varchar,workwith_gamma.dbo.decode1Cdate(j.date_time_iddoc),23)) ');
   ...
   Qr.SQL.Add('      j.ISMARK = 0                                                 ');
   Qr.SQL.Add('     and ((j.CLOSED = 5) or ((j.CLOSED = 4) and (b.sp3918<=0)))           ');
...
// Список накладаных РасходРеализхацияНДС
function GetNaklSpsNDS(Qr:TADOQuery; Apt_ID:String; D1,D2:TDateTime; Medoc:Byte=0):Boolean;
 begin
  try
   if Qr.Active then Qr.Close;
   ...
   Qr.SQL.Add('       (case when b.sp4033 in (''    1F'',''     7'',''    16'',''    2F'') or s.dt is not null then 1 else 0 end) as NReady ');
   Qr.SQL.Add('from                                 ');
   Qr.SQL.Add('     '+Opt.Gamma+'.._1sjourn j (nolock)                                                                         ');
   Qr.SQL.Add('     inner join                                                                                         ');
   Qr.SQL.Add('     '+Opt.Gamma+'..dh4028 b (nolock) on b.iddoc = j.iddoc                                                      ');
   Qr.SQL.Add('	       left join (select skl,id_apteka,dt,medoc from workwith_gamma..scan_corr group by skl,id_apteka,dt,medoc) s  ');
   Qr.SQL.Add('		               on IsNull(s.medoc,0)>='+IntToStr(Medoc)+' and b.sp9273=s.skl and b.sp4030=s.id_apteka and s.dt=convert(datetime,convert(varchar,workwith_gamma.dbo.decode1Cdate(j.date_time_iddoc),23)) ');
   Qr.SQL.Add('                                                                       ');
   Qr.SQL.Add('where j.date_time_iddoc >= '''+FormatDateTime('yyyymmdd',D1)+''' and   ');
   Qr.SQL.Add('      j.date_time_iddoc < '''+FormatDateTime('yyyymmdd',D2+1)+''' and  ');
