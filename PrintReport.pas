...
procedure TPrintRep.PrintTable(Nds:Boolean; It:Integer; Kol:Integer; SumOnly:Boolean=False; IsUkr: Boolean=false);
 ...
 Begin
  ...
  if (Sum>0) then
   ...
   if It>0 then
    ...
    if IsUkr then
      AddText('Всього відвантажено найменувань: '+IntToWordsUA(RC,0)+'.'+#10)
    else
...
//    AddText('Всего единиц товара: '+IntToWordsRU(SumKol,0)+'.'+#10);
    ...
    if NDS then
     begin
      if IsUkr then
        AddText('На суму '+CurrToWordsUA(Sum*5/6,0)+' без ПДВ'+#10)
      else
        AddText('На сумму '+CurrToWordsRU(Sum*5/6,0)+' без НДС'+#10);
      AddInterv(1);
     end else begin
               if IsUkr then
                 AddText('На суму '+CurrToWordsUA(Sum*100/107,0)+' без ПДВ'+#10)
               else
                 AddText('На сумму '+CurrToWordsRU(Sum*100/107,0)+' без НДС'+#10);
               AddInterv(1);
              end;
    if IsUkr then
      PrintRep.AddText('На суму '+CurrToWordsUA(Sum,0))
    else
      PrintRep.AddText('На сумму '+CurrToWordsRU(Sum,0));
//    if NDS then
     if IsUkr then
       PrintRep.AddText(' з урахуванням ПДВ'+#10)
     else
