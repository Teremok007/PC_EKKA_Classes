...
function PrintFMReport(pIPADDR:string;out ErrorCode,ErrorDescription:string;const RepID:Byte;BegDate:TDateTime=0;const EndDate:TDateTime=0;const BegZRep:integer=-1;const EndZRep:integer=-1):boolean;//отчеты из фискальной памяти
            ...
            begin
{
    Типы оплат:
     ...
     4 - Наличные  |   1  | Оплата Наличными
              ...
              js:=js+'{"P":{"sum":'+ss;
{
PayType: byte = 1; rrn_: integer = 0; card_: string = ''
}
              if PayType = 4 then
              begin
                      ...
                      begin
                        ...
                        begin
                          if cgi_chk.cgi_chk_object[i].F[j].P.no = 1 then
                            ...
                            ReceivedFromCustomer:=ReceivedFromCustomer+cgi_chk.cgi_chk_object[i].F[j].P.sum;//+cgi_chk.cgi_chk_object[i].F[j].D.sum;
...
//                            IssuedToCustomer:=IssuedToCustomer+cgi_chk.cgi_chk_object[i].R[j].S.sum+cgi_chk.cgi_chk_object[i].R[j].D.sum;
                            IssuedToCustomer:=IssuedToCustomer+cgi_chk.cgi_chk_object[i].R[j].P.sum;//+cgi_chk.cgi_chk_object[i].R[j].D.sum;
                      ...
                      begin
                        ...
                        begin
                          ...
                          if (cgi_chk.cgi_chk_object[i].F[j].P.no = 2)or(cgi_chk.cgi_chk_object[i].F[j].P.no = 3)or(cgi_chk.cgi_chk_object[i].F[j].P.no = 4) then
                            //ReceivedFromCustomer:=ReceivedFromCustomer+cgi_chk.cgi_chk_object[i].F[j].S.sum+cgi_chk.cgi_chk_object[i].F[j].D.sum;
                            NonCashPayment:=NonCashPayment+cgi_chk.cgi_chk_object[i].F[j].P.sum;//+cgi_chk.cgi_chk_object[i].F[j].D.sum;
                        end; //for j:=0 to length(cgi_chk.cgi_chk_object[i].F) do
                      end; //if cgi_chk.cgi_chk_object[i].F <> nil then
//
...
//                            IssuedToCustomer:=IssuedToCustomer+cgi_chk.cgi_chk_object[i].R[j].S.sum+cgi_chk.cgi_chk_object[i].R[j].D.sum;
                            NonCashRefund:=NonCashRefund+cgi_chk.cgi_chk_object[i].R[j].P.sum;//+cgi_chk.cgi_chk_object[i].R[j].D.sum;
                        end; //for j:=0 to length(cgi_chk.cgi_chk_object[i].R) do
                      end; //if cgi_chk.cgi_chk_object[i].R <> nil then
                    end;
{
...
begin
  ...
  case RepID of
    ...
    PRINTFMREPORT_FULL_BY_Z,PRINTFMREPORT_SHORT_BY_Z://отчет по номерам Z-отчетов
      begin
        if (BegZRep=-1)or(EndZRep=-1) then
        begin
          if BegZRep=-1 then
          begin
    ...
    200..299:
      begin
        ...
        begin
          try
            if IsDeflated then
              Text:=UTF8ToAnsi(Utf8Encode(copy(curr_wide,4,length(curr_wide)-3)))
            else
              Text:=UTF8ToAnsi(Utf8Encode(copy(curr_wide,0,length(curr_wide))));
            Text:=UTF8ToAnsi(Text);
            if ParsePrintFMReport(Text) then
            begin
              ErrorCode:=DEV_STATE_ERROR_CODE_CANNOT_PRINT_CHECK;
              ErrorDescription:=DEV_STATE_ERROR_DESCRIPTION_CANNOT_PRINT_CHECK;
              Result:=false;
