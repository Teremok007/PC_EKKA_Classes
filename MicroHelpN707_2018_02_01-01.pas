...
//  str_params:='?'+PRM_PRINTFMREPORT_REPID+'='+IntToStr(RepID)+
  str_params:='?'+IntToStr(RepID)+
//    '&'+PRM_PRINTFMREPORT_BEGINDATE+'='''+FormatDateTime('YYYY-MM-DD',BegDate)+''''+
    '&'+FormatDateTime('YYYY-MM-DD',BegDate)+
//    '&'+PRM_PRINTFMREPORT_ENDDATE+'='''+FormatDateTime('YYYY-MM-DD',EndDate)+''''+
    '&'+FormatDateTime('YYYY-MM-DD',EndDate)+
//    '&'+PRM_PRINTFMREPORT_BEGINZREP+'='+IntToStr(BegZRep)+
    '&'+IntToStr(BegZRep)+
//    '&'+PRM_PRINTFMREPORT_ENDZREP+'='+IntToStr(EndZRep);
    '&'+IntToStr(EndZRep);
    ...
    200..299:
      begin
        ...
        if IsDeflated then
          ...
          decodedString:=UTF8ToAnsi(Utf8Encode(copy(deflatedString,0,length(deflatedString))));
        decodedString:=UTF8ToAnsi(decodedString);
        if ParsePrintFMReport(decodedString) then
        begin
          ErrorCode:=DEV_STATE_ERROR_CODE_CANNOT_PRINT_CHECK;
          ErrorDescription:=DEV_STATE_ERROR_DESCRIPTION_CANNOT_PRINT_CHECK;
          Result:=false;
