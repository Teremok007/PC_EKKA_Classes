...
Type
  ...
  public
    ...
    function  fpOpenFiscalReceipt(Param:Byte=1; NChek:Integer=0):Boolean; override;              // Открытие нового Чека
...
{                       FP.GetLastReceiptNum;
    ...
    EKKA_N707: begin //fpGetStatus
                 ...
                 try
                   ...
                   if trim(chk_number.ErrCode)='5' then
    ...
    Case TypeEKKA of
      ...
      EKKA_N707: begin  //GetReceiptNumber
                   ...
                   begin
                     ...
                     if trim(chk_numb.ErrCode)='5' then
                       if MessageDlg('Чековая лента пустая.'+#13+'Сделать служебное внесение и/или распечатать нулевой чек?',mtWarning,[mbYes,mbNo],0)=mrYes then
                       begin
                         GetServiceShift(FConnectString, ErrCode,ErrMess);
                         chk_numb:=GetLastChkNo(IPAddr);
...
var
  ...
  ErrCode,ErrMess: string;
  c_str: string;
begin   // эта функция перекрывает область видимости свойства ReceiptNumber предка(TMarry301), для того, чтобы можно было передать параметр и определить по нему тип требуемого чека(фискальный, возвратный...)
...
function TEKKA.fpOpenFiscalReceipt(Param:Byte=1; NChek:Integer=0):Boolean;
begin
  if EmulEKKA then
    try
...
function TEKKA.fpCloseFiscalReceipt(TakedSumm: Currency; TypeOplat: Integer;
  SumCheck: Currency=0; SumB1: Currency=0; IsDnepr: Boolean=False): Boolean;
var
  PaidCode: Char;
...
//                       ShowMessage(E.Message);
    ...
    EKKA_N707: begin //fpCloseFiscalReceipt
                 ...
                 if PrintCheck(curr_check.Text,ErrorCode,ErrorDescription,curr_chek,IPAddr) then
                 begin
                   Result:=false;
...
begin
  if (Param in [1,2]) and ((AptekaID in [240]) or (IsFLP)) then
    S:='НЕ ФIСКАЛЬНИЙ ЧЕК'
  else
    Case Param of
      1: S:='ФIСКАЛЬНИЙ ЧЕК';
