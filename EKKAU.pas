                     ...
                     if not FIsConnected then
                      begin
                        ...
                        if not ics.ModemInitialize(PortNum)=0 then
                          raise Exception.Create('Ошибка модема '+IntToStr(ics.prGetModemErrorCode)+': '+ics.prGetModemErrorText);
//                        if not ics.ModemUpdateStatus then
//                          raise Exception.Create('Ошибка модема '+IntToStr(ics.prGetModemErrorCode)+': '+ics.prGetModemErrorText+' Код ошибки модема '+IntToStr(ics.stModemFailCode));
...
begin
  if (length(trim(check.ErrCode))>0)or(check.ChkNumber <= 0) then
    MessageDlg('Чековая лента пустая.'+#13+
               'Печать чеков временно не возможна!'+#13+
               'Для восстановления возможности печати чеков'+#13+
               'необходимо открыть чековую ленту.'+#13+
               ''+#13+
               'Откройте меню "Служебные" => "ОТЧЕТЫ (Z,X, внесение/выдача)..." '+#13+
               'в главном меню программы и распечатайте "Нулевой Чек" '+#13+
               'или сделайте Служебное Внесение в размере суммы последнего изъятия.',mtWarning,[mbOK],0);
end;
}
{
...
*)
          ...
          begin
            msg:='Чековая лента пустая.'+#13+
              'Печать чеков временно не возможна!'+#13+
              'Для восстановления возможности печати чеков'+#13+
              'необходимо открыть чековую ленту.'+#13+
              ''+#13+
              'Чтобы открыть чековую ленту'+#13+
              'откройте меню "Служебные" => "ОТЧЕТЫ (Z,X, внесение/выдача)..." '+#13+
              'в главном меню программы и распечатайте "Нулевой Чек" '+#13+
              'или сделайте Служебное Внесение в размере суммы '+#13+
                  ...
                  try
                    ...
                    begin
                      ...
                      try
                        ri:=TStringList.Create;
                        ri.Clear;
                        ri.Add(ics.prSerialNumber);   //0 заводской номер
                        ri.Add(ics.prFiscalNumber);   //1 фискальный номер
                        ri.Add('    ');                   //2 Наименование и адрес предприятия (всегда пусто)
                        ri.Add(FormatDateTime('YYYYMMDD',ics.prCurrentDate));   //3 Дата
                        ri.Add(FormatDateTime('HHMMSS',ics.prCurrentTime));     //4 Время
                        ri.Add(chr(1)); //5 Положение системного ключа (всегда 1)
                        ri.Add('1'); //6 Признак ожидаемой команды (всегда 1)
                        ...
                        ri.Add(IntToStr(ics.stModemPID_LastWrite));//11 номер последнего успешно созданного чека
                        ri.Add('');//12 идентификатор последней успешно исполненной команды
                        ri.Add(ics.prHardwareVersion);//13 идентификатор версии ПО ЭККР;
                        ri.Add('');//14 дата создания версии ПО ЭККР в формате ггггммдд;
                        ri.Add('');//15 текущая информационная строка чека
                        ri.Add(FormatDateTime('YYYYMMDD',ics.prDateFiscalization));//16 дата программирования валюты ЭККР в формате ггггммдд
                        ri.Add('2');//17 количество знаков после десятичной точки в изображении сумм
                        ri.Add('грн');//18 сокращенное наименование валюты ЭККР
                        FRD_Item.Text:=ri.Text;
                        FRD_Item[9]:=Chr(StrToInt(sm));
                        Result:=True;
                        FLastError:='';
                  ...
                  try
                    ...
                    begin
                      ...
                      if ics.stModemFailCode <> 0 then
                        raise Exception.Create('Ошибка модема '+IntToStr(ics.prGetModemErrorCode)+': '+ics.prGetModemErrorText+'(Код ошибки: '+IntToStr(ics.stModemFailCode)+')');
...
{
    Типы оплат:
          Мария    | Датекс FP3530 | Exellio LP1000 | Exellio FP2000 | N707 | ИКС810Т | Описание
     1 - Безнал 3  |      N        |       2        |        4       |   3  |    2    | Оплата в кредит
     2 - Безнал 2  |      D        |       4        |        3       |   4  |    1    | Оплата с помощью платежной карты
     3 - Безнал 1  |      C        |       3        |        2       |   2  |    3    | Оплата чеком
     4 - Наличные  |      P        |       1        |        1       |   1  |    4    | Оплата Наличными
}
  case TypeEKKA of
...
{
    Типы оплат:
          Мария    | E810T| Описание
     1 - Безнал 3  |   2  | Оплата в кредит
     2 - Безнал 2  |   1  | Оплата с помощью платежной карты
     3 - Безнал 1  |   3  | Оплата чеком
     4 - Наличные  |   4  | Оплата Наличными
}
                  FLastError:='';
                  try
                    case TypeOplat of
                      1:Ty:=2;//Оплата в кредит
                      2:Ty:=1;//Оплата с помощью платежной карты
                      3:Ty:=3;//Оплата чеком
                      4:Ty:=4;//Оплата Наличными
                    end;
                    if IsDnepr then TyB1:=2 else TyB1:=3;
                    ...
                    if FIsConnected then
                     begin
                      ...
                      if not Result then raise Exception.Create(ics.prGetErrorText+'. Код ошибки '+IntToStr(ics.prGetResultByte)+#13+'Статус '+IntToStr(ics.prGetStatusByte)+' Дополнительные флаги '+IntToStr(ics.prGetReserveByte));
                      if SumB1=0 then
                       begin
                        case Ty of
                         1: Result:=ics.FPPaymentByCard(Ty, ics.prSumTotal,true,true,BankCard,IntToStr(RRN)); //Оплата с помощью платежной карты
                         2: Result:=ics.FPPayment(Ty, ics.prSumTotal,true,true,''); //Оплата в кредит
                         3: Result:=ics.FPPayment(Ty, ics.prSumTotal,true,true,''); //Оплата чеком
                         4: Result:=ics.FPPayment(Ty, ics.prSumTotal,true,true,''); //Оплата Наличными
                        end;
                        if not Result then raise Exception.Create(ics.prGetErrorText+'. Код ошибки '+IntToStr(ics.prGetResultByte)+#13+'Статус '+IntToStr(ics.prGetStatusByte)+' Дополнительные флаги '+IntToStr(ics.prGetReserveByte));
                       end else begin
    ...
    EKKA_E810T: begin //fpCashState
                  ...
                  try
                    ...
                    begin
                      ...
                      begin
                        FRD_Item.Clear;
                        FRD_Item.Add(IntToStr(0)); //0 начальный остаток
                        FRD_Item.Add(IntToStr(ics.prDayCashInSum)); //1 служебное внесение
                        FRD_Item.Add(IntToStr(ics.prDayCashOutSum)); //2 служебное изъятие
                        FRD_Item.Add(IntToStr(ics.prDaySaleSumOnPayForm4));//3 получено от клиента нал.
                        FRD_Item.Add(IntToStr(ics.prDayRefundSumOnPayForm4));//4 выдано клиенту нал.
                        FRD_Item.Add(IntToStr(ics.prDayCashInSum-ics.prDayCashOutSum+ics.prDaySaleSumOnPayForm4-ics.prDayRefundSumOnPayForm4));//5 конечный остаток
                        FRD_Item.Add(IntToStr(ics.prDaySaleSumOnPayForm1+ics.prDaySaleSumOnPayForm2+ics.prDaySaleSumOnPayForm3));//6 безналичная оплата
                        FRD_Item.Add(IntToStr(ics.prDayRefundSumOnPayForm1+ics.prDayRefundSumOnPayForm2+ics.prDayRefundSumOnPayForm3));//7 безналичный возврат
                      end
                      else
                        raise Exception.Create(ics.prGetErrorText+'. Код ошибки '+IntToStr(ics.prGetResultByte)+#13+'Статус '+IntToStr(ics.prGetStatusByte)+' Дополнительные флаги '+IntToStr(ics.prGetReserveByte));
                    end
                  ...
                  try
                    ...
                    begin
                      ...
                      begin
                        ...
                        FRD_Item.Add(IntToStr(ics.prDaySaleSumOnTax1)); //1  - продажи по ставке А20%
                        FRD_Item.Add(IntToStr(ics.prDaySaleSumOnTax2)); //2  - продажи по ставке Б7%
                        FRD_Item.Add(IntToStr(ics.prDaySaleSumOnTax3)); //3  - продажи по ставке В0%
                        FRD_Item.Add(IntToStr(ics.prDaySaleSumOnTax4)); //4  - продажи по ставке Г
                        FRD_Item.Add(IntToStr(ics.prDaySaleSumOnTax5)); //5  - продажи по ставке Д
                        FRD_Item.Add(IntToStr(ics.prDaySaleSumOnTax6)); //6  - продажи по ставке Е
                        FRD_Item.Add(IntToStr(0)); //7  - продажи по ставке Ж
                        FRD_Item.Add(IntToStr(0)); //8  - продажи по ставке З
                        FRD_Item.Add(IntToStr(0)); //9  - 0
                        FRD_Item.Add(IntToStr(0)); //10 - 0
                        FRD_Item.Add(IntToStr(ics.prDayRefundSumOnTax1)); //11 - возвраты покупателям по ставке А20%
                        FRD_Item.Add(IntToStr(ics.prDayRefundSumOnTax2)); //12 - возвраты покупателям по ставке Б7%
                        FRD_Item.Add(IntToStr(ics.prDayRefundSumOnTax3)); //13 - возвраты покупателям по ставке В0%
                        FRD_Item.Add(IntToStr(ics.prDayRefundSumOnTax4)); //14 - возвраты покупателям по ставке Г
                        FRD_Item.Add(IntToStr(ics.prDayRefundSumOnTax5)); //15 - возвраты покупателям по ставке Д
                        FRD_Item.Add(IntToStr(ics.prDayRefundSumOnTax6)); //16 - возвраты покупателям по ставке Е
                        FRD_Item.Add(IntToStr(0)); //17 - возвраты покупателям по ставке Ж
                        FRD_Item.Add(IntToStr(0)); //18 - возвраты покупателям по ставке З
                      end
                      else
                        raise Exception.Create(ics.prGetErrorText+'. Код ошибки '+IntToStr(ics.prGetResultByte)+#13+'Статус '+IntToStr(ics.prGetStatusByte)+' Дополнительные флаги '+IntToStr(ics.prGetReserveByte));
                    end
...
begin
  ...
  begin
    try
      ...
      MrFont.AddStr(' МФО  - '+MFO,0);
      MrFont.AddStr(' ОКПО - '+OKPO,0);
      MrFont.AddStr(' р/р  - '+R_R,0);
      MrFont.AddStr(' чек №  '+IntToStr(NumbChek),0);
      MrFont.AddStr(' сума   '+CurrencyToStr(SumChek),0);
      MrFont.AddStr(' ',0);
      bPrintFooter(4);
    except
      on E:Exception do
    ...
    EKKA_MARRY301MTM:
      begin//Result:=inherited fpCloseServiceReceipt;
        try
          ...
          s:='TEXT000 р/р  - '+R_R;
          fpSendCommand(s);
          s:='TEXT000 чек №  '+IntToStr(NumbChek);
          fpSendCommand(s);
          s:='TEXT000 сума   '+CurrencyToStr(SumChek);
          fpSendCommand(s);
          s:='TEXT000 ';
          fpSendCommand(s);
          s:='DBEG';
