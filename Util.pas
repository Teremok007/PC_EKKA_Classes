...
// возвращает true если бинарное значение - null
function binaryIsNull(s:String):Boolean;

// Преобразование поля Text в int, возвращает 0 если невозможно преобразовать
function getValueEdInt(ed:TCustomEdit):Integer;

// Преобразование поля Text в currency, возвращает 0 если невозможно преобразовать
function getValueEdCurr(ed:TCustomEdit):Currency;

// Рисование иконки в ячейке DBGrid по координатам x,y
procedure drawIconInDBGridXY(cvDest:TCanvas; imSource:TImage; x,y:Integer);

// Рисование иконки в ячейке DBGrid позиционно
procedure drawIconInDBGridPos(cvDest:TCanvas; imSource:TImage; r:TRect; p:Byte; dx:Integer=0);
...
function RoundAmount(aAmount : Currency): Currency;
var
  iPenny,iTenth : integer;
begin
  iPenny := Round(Int(StrToFloat(FloatToStr(Frac(aAmount)*100)))); // Копейки
  { сотая часть копеек }
  iTenth := Round(Frac(iPenny/10)*10);
  { Десятая часть копеек }
  iPenny := Round(Int(iPenny/10)*10);
  { обнуляем сотые копееки + округляем }
  iPenny := IfThen(iTenth >= 5,iPenny+10,iPenny);
  { формируем сумму из целой части + дробной части }
...
var bm:TBitMap;
 begin
  if Assigned(imSource)=false then exit;
  bm:=TBitMap.Create;
  try
   bm.Assign(imSource.Picture);
   bm.Transparent:=True;
...
{
 значения p:
  1: позиционирование по левому краю
  2: позиционирование по правому краю

  dx: смещение от левого или правого края в зависимости от параметра p
}
 begin
  if Assigned(imSource)=false then exit;
  if not (p in [1,2]) then exit;
  case p of
   1:begin
      x:=r.Left+3+dx;
      y:=((r.Bottom-r.Top)-imSource.Height) div 2;
     end;
   2:begin
      x:=r.Right-3-dx-imSource.Width;
      y:=r.Top+(((r.Bottom-r.Top)-imSource.Height) div 2);
     end;
  end;
  drawIconInDBGridXY(cvDest,imSource,x,y);
 end;
