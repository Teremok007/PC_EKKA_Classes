...
//       procedure RefreshImages;
     ...
     public
       ...
       procedure PrintCennik(Firm:String; NDS:Boolean; K:Boolean);
       procedure PrintTable(Nds:Boolean; It:Integer);
...
procedure TPrintRep.PrintCennik(Firm:String; NDS:Boolean; K:Boolean);
var i,j,RC:Integer;
    Bm:TBitMap;

 procedure PrepareCennik(Cl:TCell; Cap:String; A:Integer; N:String; C:Currency; Kod:Integer; EAN13:String);
 var Tb:TTableObj;
     Br:TBorder;
  ...
  begin
   ...
   NumStr:=3;
   if K then Inc(NumStr);
   if NDS then begin if Not(K) then Inc(NumStr); Inc(NumStr,2); NStrEAN:=NumStr-3; end
          else begin Inc(NumStr); NStrEAN:=NumStr-1; end;
   Cl.AddTable(2,NumStr);
   Tb:=Cl.LastTable;
   Tb.SetBorders(1,1,2,NumStr,EMPTY_BORDER);

   Tb.MergeCells(1,2,2,2);
//   Tb.MergeCells(1,NStrEAN,2,NStrEAN);
   Tb.Cell[1,1].AddText(Cap);

   Tb.Cell[2,1].Align:=AL_RIGHT;
   Tb.Cell[2,1].Font.Size:=5;
   Tb.Cell[2,1].Font.Style:=[fsBold];
   Tb.Cell[2,1].AddText(IntToStr(A));

   Tb.Cell[1,2].Font.Size:=5;
   Tb.Cell[1,2].Align:=AL_CENTER;
   Tb.Cell[1,2].AddText(N);
   if K then
    begin
     Tb.MergeCells(1,3,2,3);
     Tb.Cell[1,3].Font.Size:=4;
     Tb.Cell[1,3].Font.Style:=[];
     Tb.Cell[1,3].Align:=AL_RIGHT;
     Tb.Cell[1,3].AddText(IntToStr(Kod));
    end;
   if NDS then
    begin
     cNDS:=C/6;
     Tb.Cell[1,NumStr-2].Font.Size:=4;
     Tb.Cell[1,NumStr-2].Font.Style:=[fsBold];
     Tb.Cell[1,NumStr-2].Align:=AL_RIGHT;
     Tb.Cell[1,NumStr-2].AddText('Цена без НДС');

     Tb.Cell[1,NumStr-1].Font.Size:=4;
     Tb.Cell[1,NumStr-1].Font.Style:=[fsBold];
     Tb.Cell[1,NumStr-1].Align:=AL_RIGHT;
     Tb.Cell[1,NumStr-1].AddText('НДС');

     Tb.Cell[2,NumStr-2].Font.Size:=4;
     Tb.Cell[2,NumStr-2].Font.Style:=[fsBold];
     Tb.Cell[2,NumStr-2].Align:=AL_RIGHT;
     Tb.Cell[2,NumStr-2].AddText(CurrToStrF(C-cNDS,ffFixed,2)+' грн');

     Tb.Cell[2,NumStr-1].Font.Size:=4;
     Tb.Cell[2,NumStr-1].Font.Style:=[fsBold];
     Tb.Cell[2,NumStr-1].Align:=AL_RIGHT;
     Tb.Cell[2,NumStr-1].AddText(CurrToStrF(cNDS,ffFixed,2)+' грн');

     Tb.Cell[1,NumStr].Font.Size:=5;
     Tb.Cell[1,NumStr].Font.Style:=[fsBold];
     Tb.Cell[1,NumStr].Align:=AL_RIGHT;
     Tb.Cell[1,NumStr].AddText('Цена с НДС');
     Tb.Cell[1,NumStr].TopBorder:=Border(clBlack,1,psDot);
     Tb.Cell[2,NumStr].TopBorder:=Border(clBlack,1,psDot);
    end;

   Tb.Cell[2,NumStr].Font.Size:=5;
   Tb.Cell[2,NumStr].Font.Style:=[fsBold];
   Tb.Cell[2,NumStr].Align:=AL_RIGHT;
   Tb.Cell[2,NumStr].AddText(CurrToStrF(C,ffFixed,2)+' грн');

   // Добавление даты и Штрих-Кода
   Tb.Cell[2,NStrEAN].Font.Size:=3;
   Tb.Cell[2,NStrEAN].AddText(DateToStr(Date)+#10);
   if Length(EAN13)=12 then
    try
     ...
     try
      Bm.Width:=400;
      Bm.Height:=50;
      DrawBarCode(Bm.Canvas,EAN13,12,0,0,Round(Bm.Width*0.50),Round(Bm.Height*0.7));
      Tb.Cell[1,NStrEAN].Stretch:=True;
      Tb.Cell[1,NStrEAN].AddImage(Bm,40);
     finally
      Bm.Free;
     end;
    except
   ...
   begin
    for i:=1 to 3 do
     begin
      ...
      PrepareCennik(PrintRep.LastTable.Cell[i,j],
                    Firm,
                    Qr.FieldByName('Art_Code').AsInteger,
                    TrimRight(Qr.FieldByName('Names').AsString),
                    Qr.FieldByName('Cena').AsCurrency,
                    0, //Qr.FieldByName('Kod').AsInteger,
                    Copy(Qr.FieldByName('EAN13').AsString,1,12)
                   );
      Qr.Next;
     end;
    if Qr.Eof then Break;
...
procedure TPrintRep.PrintTable(Nds:Boolean; It:Integer);
