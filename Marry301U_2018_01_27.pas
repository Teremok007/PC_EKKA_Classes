...
Type
 ...
 public
  ...
  function  fpCheckCopy(Cnt:Byte=1):Boolean; virtual;                      // Печать копии чека
  ...
  function  fpClearServiceText:Boolean;              // отмена  текстовой информации
  function  fpCancelServiceReceipt:Boolean;          // отмена служебного чека
  function  fpOpenServiceReceipt:Boolean;            // открытие служебного чека
  function  fpCloseServiceReceipt:Boolean;           //  закрытие служебного чека
  ...
  Repeat
   if ReadString(RS) then
    begin
     ...
     if ((ss1='SOFT') or (ss1='HARD') or (ss1='MEM_') or (ss1='RTC_') or (ss1='ERRO')) and (AnsiUpperCase(Copy(Res,1,9))<>'SOFTPAPER') and (AnsiUpperCase(Copy(Res,1,9))<>'HARDPOINT') then FLastError:=Res else
...
function TMarry301.fpCloseFiscalReceipt(TakedSumm:Currency; TypeOplat:Integer; SumCheck:Currency=0; SumB1:Currency=0; IsDnepr:Boolean=False; ControlStreem:Byte=0):Boolean;
var S:String;
    TOpl,Sm,Sm1:String;
    T:TDateTime;
    B:Boolean;
...
function TMarry301.fpCheckCopy(Cnt:Byte=1):Boolean;
 begin
